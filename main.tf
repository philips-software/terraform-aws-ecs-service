...
locals {
  target_group_arn = "${var.target_group_arn == "" ? element(concat(aws_alb_target_group.target_group.*.arn, list("")), 0) : var.target_group_arn}"
}
...
resource "aws_ecs_service" "service_alb" {
  ...
  depends_on = ["null_resource.ecs_services_dependencies", "aws_alb_listener.listener", "aws_alb_target_group.target_group", "aws_lb_listener_rule.default"]
